...
class PDFTool {
  constructor() {
    ...
    const stepVal = () =>
      Math.max(1, parseInt(this.el.inputGStep?.value || "1", 10));
    this.el.btnGPrev &&
      (this.el.btnGPrev.onclick = () => this.applyGlobalOffset(-stepVal()));
    this.el.btnGNext &&
      (this.el.btnGNext.onclick = () => this.applyGlobalOffset(stepVal()));
    this.el.btnGPrevJump &&
      (this.el.btnGPrevJump.onclick = () => {
        const grid = this.computeGrid ? this.computeGrid() : { cols: 1 };
        const cols = grid.cols || 1;
        const n =
          (this.getTargetSlotIndices
            ? this.getTargetSlotIndices(cols).length
            : this.slots) || 1;
        this.applyGlobalOffset(-n);
      });
    this.el.btnGNextJump &&
      (this.el.btnGNextJump.onclick = () => {
        const grid = this.computeGrid ? this.computeGrid() : { cols: 1 };
        const cols = grid.cols || 1;
        const n =
          (this.getTargetSlotIndices
    ...
    this.el.btnGApply &&
      (this.el.btnGApply.onclick = () =>
        this.applyGlobalAbsolute(
          parseInt(this.el.inputGPage?.value || "1", 10),
        ));
    this.el.btnGClear &&
      (this.el.btnGClear.onclick = () => {
        this.selectedSlots.clear();
        this.el.btnSelAllSlots &&
          (this.el.btnSelAllSlots.onclick = () => this.selectAllSlots());
        this.el.btnDeselAllSlots &&
          (this.el.btnDeselAllSlots.onclick = () => this.deselectAllSlots());
        this.el.btnSelN &&
          (this.el.btnSelN.onclick = () => this.selectNSlotsFromActive());
        this.el.btnDeselN &&
          (this.el.btnDeselN.onclick = () => this.deselectNSlotsFromActive());
  ...
  setOCRStatus(t) {
    this.el.ocrStatus.textContent = t;
  }
  ...
  syncUI() {
    ...
    if (this.el.renderZoomLabel)
      this.el.renderZoomLabel.textContent = `${Math.round((this.renderZoom || 1) * 100)}%`;
    if (this.el.inputRenderZoom)
      this.el.inputRenderZoom.value = Math.round((this.renderZoom || 1) * 100);
  }
  ...
  visiblePageIndices(doc) {
    ...
    const set = new Set(
      doc.filter.map((n) => n - 1).filter((i) => i >= 0 && i < total),
    );
  ...
  bind() {
    ...
    const applyPresentation = () => {
      ...
      const label = this.presentationMode
        ...
        : "Mode présentation";
      if (this.el.btnTogglePresentation)
        this.el.btnTogglePresentation.textContent = label;
      if (this.el.btnTogglePresentation2)
        this.el.btnTogglePresentation2.textContent = label;
    };
    this.el.btnTogglePresentation &&
      (this.el.btnTogglePresentation.onclick = () => {
        this.presentationMode = !this.presentationMode;
        applyPresentation();
      });
    this.el.btnTogglePresentation2 &&
      (this.el.btnTogglePresentation2.onclick = () => {
        this.presentationMode = !this.presentationMode;
        applyPresentation();
      });
    ...
    const syncRenderZoom = () => {
      if (this.el.renderZoomLabel)
        this.el.renderZoomLabel.textContent = `${Math.round(this.renderZoom * 100)}%`;
    };
    if (this.el.inputRenderZoom) {
      this.el.inputRenderZoom.oninput = () => {
        this.renderZoom =
          (parseInt(this.el.inputRenderZoom.value, 10) || 100) / 100;
        syncRenderZoom();
        this.render();
      };
    }
    ...
    // Presets
    this.el.btnExportPreset &&
      (this.el.btnExportPreset.onclick = () => this.exportPresetJSON());
    this.el.btnImportPreset &&
      (this.el.btnImportPreset.onclick = () => this.importPresetJSON());
    ...
    const applySlotChrome = () => {
      ...
      if (this.el.btnToggleSlotChrome)
        this.el.btnToggleSlotChrome.textContent = label;
      if (this.el.btnToggleSlotChrome2)
        this.el.btnToggleSlotChrome2.textContent = label;
    };
    this.el.btnToggleSlotChrome &&
      (this.el.btnToggleSlotChrome.onclick = () => {
        this.showSlotChrome = !this.showSlotChrome;
        applySlotChrome();
      });
    this.el.btnToggleSlotChrome2 &&
      (this.el.btnToggleSlotChrome2.onclick = () => {
        this.showSlotChrome = !this.showSlotChrome;
        applySlotChrome();
      });

    // Presentation exit (floating)
    this.el.btnExitPresentation &&
      (this.el.btnExitPresentation.onclick = () => {
        this.presentationMode = false;
    ...
    this.el.btnSlotsMinus.onclick = () => {
      this.el.inputSlots.value = clamp(
        (parseInt(this.el.inputSlots.value, 10) || 1) - 1,
        1,
        9999,
      );
    ...
    this.el.inputSlotWDefault.onchange = () => {
      this.slotDefaultW = Math.max(
        200,
        parseInt(this.el.inputSlotWDefault.value, 10) || 595,
      );
      this.el.inputSlotWDefault.value = this.slotDefaultW;
      this.render();
    };
    this.el.inputSlotHDefault.onchange = () => {
      this.slotDefaultH = Math.max(
        200,
        parseInt(this.el.inputSlotHDefault.value, 10) || 842,
      );
    ...
    this.el.inputZoom.oninput = () => {
      ...
      document.documentElement.style.setProperty(
        "--gap",
        `${clamp(Math.round(12 + (this.globalZoom - 1) * 20), 10, 44)}px`,
      );
      this.render();
    };
    ...
    this.el.btnOCRPanel.onclick = () =>
      this.el.ocrPanel.classList.toggle("active");
    this.el.btnOCRClose.onclick = () =>
      this.el.ocrPanel.classList.remove("active");
    this.el.btnOCRCopy.onclick = async () => {
      try {
        await navigator.clipboard.writeText(this.el.ocrText.value || "");
      } catch (e) {
        alert("Copie impossible (permissions navigateur).");
      }
    };
    this.el.btnOCRClear.onclick = () => {
      this.el.ocrText.value = "";
      this.setOCRStatus("Prêt");
    };

    this.el.btnZoneToggle.onclick = () => {
      this.zoneEnabled = !this.zoneEnabled;
      this.el.btnZoneToggle.textContent = this.zoneEnabled
        ? "Zones: ON"
        : "Zones: OFF";
      this.render();
    };
    ...
    input.onchange = async () => {
      ...
      for (const file of files) {
        try {
          ...
          if (off < 0) {
            alert(
              `"${file.name}" n'est pas un PDF valide (%PDF- introuvable).`,
            );
            continue;
          }
          ...
          try {
            ...
            pdfLibDoc = isPdfBytes(norm)
              ? await PDFLib.PDFDocument.load(norm, { ignoreEncryption: true })
              : null;
          } catch (e) {
            console.warn(
              "PDF-Lib a échoué (édition/export désactivés pour ce fichier).",
              e,
            );
            pdfLibDoc = null;
          }
          ...
          try {
            doc.fingerprint = await sha256Hex(normalizePdfBytes(bytes));
          } catch (e) {
  ...
  async addPDFToActiveDoc() {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const off = findPdfHeaderOffset(bytes);
        if (off < 0) {
          alert(`"${file.name}" n'est pas un PDF valide.`);
          return;
        }
        ...
        try {
          srcDoc.fingerprint = await sha256Hex(normalizePdfBytes(bytes));
        } catch (e) {
          srcDoc.fingerprint = null;
        }
  ...
  gotoRef(target, slotIndex = 0) {
    ...
    if (!doc) {
      alert("Document cible introuvable.");
      return;
    }
    ...
    const pageIndex = doc.pages.findIndex(
      (p) =>
        p.sourceDocId === target.sourceDocId &&
        p.sourcePageNumber === target.sourcePageNumber,
    );
    if (pageIndex < 0) {
      alert("Page cible introuvable dans le document.");
      return;
    }
  ...
  saveSlotScroll() {
    ...
    wraps.forEach((w) => {
      ...
      if (Number.isFinite(s))
        this.slotScroll[s] = { left: w.scrollLeft, top: w.scrollTop };
    });
  }
  restoreSlotScroll() {
    const wraps = this.el.viewer.querySelectorAll("[data-slot-wrap]");
    wraps.forEach((w) => {
      ...
      if (st) {
        w.scrollLeft = st.left || 0;
        w.scrollTop = st.top || 0;
      }
    });
  }
  ...
  async render() {
    ...
    for (let s = 0; s < this.slots; s++) {
      ...
      slot.className =
        "slot" +
        (s === this.activeSlot ? " active" : "") +
        (this.selectedSlots.has(s) ? " sel" : "");
      slot.onclick = (ev) => {
      ...
      // actions prev/next
      slotHead.querySelector('[data-act="prev"]').onclick = (e) => {
        e.stopPropagation();
        this.slotCursor[s] = clamp(
          this.slotCursor[s] - 1,
          0,
          visible.length - 1,
        );
      ...
      slotHead.querySelector('[data-act="next"]').onclick = (e) => {
        e.stopPropagation();
        this.slotCursor[s] = clamp(
          this.slotCursor[s] + 1,
          0,
      ...
      // selection
      slotHead.querySelector('[data-act="toggleSel"]').onclick = (e) => {
        e.stopPropagation();
        if (doc.selected.has(pageIndex)) doc.selected.delete(pageIndex);
        else doc.selected.add(pageIndex);
        this.syncSelectionPill();
      ...
      // rotations
      slotHead.querySelector('[data-act="rotL"]').onclick = (e) => {
        e.stopPropagation();
        ref.rotation = degreesNorm90(ref.rotation - 90);
        this.renderPagesList();
        this.render();
      };
      slotHead.querySelector('[data-act="rotR"]').onclick = (e) => {
        e.stopPropagation();
        ref.rotation = degreesNorm90(ref.rotation + 90);
        this.renderPagesList();
        this.render();
      ...
      slotHead.querySelector('[data-act="slotZoomMinus"]').onclick = (e) => {
        e.stopPropagation();
        applySlotZoom(Math.round((this.slotZooms[s] || 1) * 100 - 10));
      };
      slotHead.querySelector('[data-act="slotZoomPlus"]').onclick = (e) => {
        e.stopPropagation();
        applySlotZoom(Math.round((this.slotZooms[s] || 1) * 100 + 10));
      };
      ...
      slotHead.querySelector('[data-act="resetSlotZoom"]').onclick = (e) => {
        e.stopPropagation();
        this.slotZooms[s] = 1.0;
        this.render();
      };
      slotHead.querySelector('[data-act="resetSlotRot"]').onclick = (e) => {
        e.stopPropagation();
        ref.rotation = 0;
        this.renderPagesList();
        this.render();
      ...
      const renderZoneListUI = () => {
        ...
        if (!anno.zones.length) {
          ...
          for (const z of anno.zones) {
            ...
            item.className =
              "zoneItem" + (z.id === this.activeZoneBySlot[s] ? " active" : "");
            const lastNote =
              z.notes && z.notes.length ? z.notes[z.notes.length - 1] : null;
            item.innerHTML = `
              <div class="left">
            ...
            item.onclick = (e) => {
              ...
              const exists =
                wanted && (z.notes || []).some((n) => n.id === wanted);
              if (!exists) {
                const last =
                  z.notes && z.notes.length
                    ? z.notes[z.notes.length - 1].id
                    : "";
            ...
            item.querySelector('[data-act="del"]').onclick = (e) => {
              ...
              if (this.activeZoneBySlot[s] === z.id) {
                this.activeZoneBySlot[s] = "";
                this.activeNoteBySlot[s] = "";
              }
              this.render();
        ...
        const note =
          z && this.activeNoteBySlot[s]
            ? this.findNote(z, this.activeNoteBySlot[s])
            : null;
        ...
        ta.placeholder = z
          ? "Écrire une note pour la zone…"
          : "Sélectionne une zone pour écrire une note…";
        ta.disabled = !z;
        ...
        q("delNote").onclick = (e) => {
          ...
          this.links = this.links.filter(
            (L) => L.from.noteId !== note.id && L.to.noteId !== note.id,
          );
          z.notes = (z.notes || []).filter((n) => n.id !== note.id);
          const last =
            z.notes && z.notes.length ? z.notes[z.notes.length - 1].id : "";
          this.activeNoteBySlot[s] = last;
          this.render();
        };
        ...
        const outLinks = note
          ? this.links.filter((L) => L.from.noteId === note.id)
          : [];
        const inLinks = note
          ? this.links.filter((L) => L.to.noteId === note.id)
          : [];
        ...
        const addLinkItem = (L, kind) => {
          ...
          it.querySelector('[data-act="go"]').onclick = (e) => {
            ...
            const slotRaw = prompt(
              "Ouvrir dans quel slot ? (1..n)",
              String(this.activeSlot + 1),
            );
            const slotIdx = clamp(
              (parseInt(slotRaw || "", 10) || 1) - 1,
              0,
              this.slots - 1,
            );
      ...
      (async () => {
        try {
          ...
          Array.from(overlay.querySelectorAll(".zoneBox")).forEach((n) =>
            n.remove(),
          );
          ...
          const showZones =
            !!this.zoneEnabled && (this.slotShowZones[s] ?? true);
          if (!showZones) {
            return;
          }
          for (const z of zones) {
            ...
            box.className =
              "zoneBox" +
              (z.id === (this.activeZoneBySlot[s] || "") ? " active" : "");
            const lbl = document.createElement("div");
            lbl.className = "zoneLabel";
  ...
  openPicker(mode) {
    ...
    this.el.modalTitle.textContent =
      mode === "notes" ? "Toutes les notes" : "Choisir une note cible";
  ...
  exportNotesJSON() {
    ...
    downloadText(
      text,
      `notes_session_${new Date().toISOString().slice(0, 10)}.json`,
    );
  }
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        for (const idoc of importedDocs) {
          let cur = null;
          if (idoc.fingerprint && byFp.has(idoc.fingerprint))
            cur = byFp.get(idoc.fingerprint);
          if (!cur && idoc.name && byName.has(idoc.name))
            cur = byName.get(idoc.name);
        ...
        for (const idoc of importedDocs) {
          const curId =
            idoc.fingerprint && byFp.has(idoc.fingerprint)
              ? byFp.get(idoc.fingerprint).id
              : idoc.name && byName.has(idoc.name)
                ? byName.get(idoc.name).id
                : null;
          if (!curId) continue;
          ...
          curDoc.annotations = this.mergeAnnotations(
            curDoc.annotations || {},
            remapped,
          );
        }
        ...
        for (const Lraw of importedLinks) {
          ...
          const remapEnd = (end) => {
            ...
            if (end.docFp && byFp.has(end.docFp))
              docId = byFp.get(end.docFp).id;
            else if (docId && idMap.has(docId)) docId = idMap.get(docId);

            if (end.sourceFp && byFp.has(end.sourceFp))
              srcId = byFp.get(end.sourceFp).id;
  ...
  getSelectedSorted(doc) {
    const arr = Array.from(doc.selected).filter(
      (i) => i >= 0 && i < doc.pages.length,
    );
    arr.sort((a, b) => a - b);
    return arr;
  }
  ...
  async extractSelected() {
    ...
    for (const idx of sel) {
      ...
      if (!srcLib) {
        alert(
          "Export impossible: PDF-Lib indisponible pour un des documents source.",
        );
        return;
      }
  ...
  async ocrPageByRef(doc, pageIndex, zoneRectNorm = null, slotScale = 1.0) {
    ...
    if (zoneRectNorm) {
      ...
      crop
        .getContext("2d")
        .drawImage(off, sx, sy, sw, sh, 0, 0, crop.width, crop.height);
      img = crop;
    }
  ...
  async ocrDocument() {
    ...
    for (let i = 0; i < visible.length; i++) {
      const pageIndex = visible[i];
      this.setOCRStatus(
        `OCR page ${pageIndex + 1} (${i + 1}/${visible.length})…`,
      );
      const text = await this.ocrPageByRef(
        doc,
        pageIndex,
        null,
        this.globalZoom,
  ...
  async ocrVisibleSlots() {
    ...
    for (let i = 0; i < uniq.length; i++) {
      ...
      this.setOCRStatus(
        `OCR slot page ${pageIndex + 1} (${i + 1}/${uniq.length})…`,
      );
      const text = await this.ocrPageByRef(
        doc,
        pageIndex,
    ...
    this.el.ocrText.value =
      (this.el.ocrText.value ? this.el.ocrText.value + "\n\n" : "") +
      out.trim();
    this.setOCRStatus("Terminé");
  }
  ...
  async ocrSelectedPages() {
    ...
    for (let i = 0; i < sel.length; i++) {
      ...
      this.setOCRStatus(
        `OCR sélection page ${pageIndex + 1} (${i + 1}/${sel.length})…`,
      );
      const text = await this.ocrPageByRef(
        doc,
        pageIndex,
  ...
  async ocrZonesActiveSlot() {
    ...
    if (!zones.length) {
      alert(
        "Aucune zone sur la page du slot actif. Active Zones:ON puis trace une zone.",
      );
      return;
    }
...
function buildHeaderMenus() {
  ...
  if (!titleEl) {
    ...
    if (titleEl) titleEl.classList.add("appTitle");
  }
  if (!titleEl) {
...
function buildSidebarAccordions() {
  ...
  const defaultOpenTitles = new Set([
    "Affichage des pages (filtre)",
    "Mode de visualisation",
    "Zoom",
    "Pages (document actif)",
    "Notes (zone active)",
  ]);
    ...
    if (n.tagName === "H3") {
      ...
      head.onclick = () => {
        if (!allowMultipleOpen) {
          sidebar.querySelectorAll(".acc.open").forEach((a) => {
...
PDFTool.prototype.getTargetSlotIndices = function (cols) {
  ...
  if (scope === "selected" && this.selectedSlots && this.selectedSlots.size) {
    return Array.from(this.selectedSlots).filter(
      (i) => i >= 0 && i < this.slots,
    );
  }
...
PDFTool.prototype.syncGlobalNavChips = function (cols) {
  ...
  const rowsEl = this.el.chipsRows,
    colsEl = this.el.chipsCols;
  ...
  for (let r = 0; r < rows; r++) {
    ...
    d.className =
      "chip" + (this.selectedRows && this.selectedRows.has(r) ? " on" : "");
    d.textContent = "L" + (r + 1);
    d.onclick = () => {
      if (!this.selectedRows) this.selectedRows = new Set();
      if (this.selectedRows.has(r)) this.selectedRows.delete(r);
      else this.selectedRows.add(r);
      this.syncGlobalNavChips(cols);
    };
  ...
  for (let c = 0; c < cols; c++) {
    const d = document.createElement("div");
    d.className =
      "chip" + (this.selectedCols && this.selectedCols.has(c) ? " on" : "");
    d.textContent = "C" + (c + 1);
    d.onclick = () => {
      if (!this.selectedCols) this.selectedCols = new Set();
      if (this.selectedCols.has(c)) this.selectedCols.delete(c);
      else this.selectedCols.add(c);
      this.syncGlobalNavChips(cols);
    };
...
PDFTool.prototype.applyGlobalOffset = function (delta) {
  ...
  targets.forEach((s) => {
    const cur =
      this.slotCursor && this.slotCursor[s] != null ? this.slotCursor[s] : 0;
...
PDFTool.prototype.applySelectedPagesToSlots = function () {
  ...
  const targets =
    (this.getTargetSlotIndices
      ? this.getTargetSlotIndices(cols)
      : Array.from({ length: this.slots }, (_, i) => i)) || [];

  const scope = this.globalScope || "all";
  if (
    scope === "selected" &&
    (!this.selectedSlots || !this.selectedSlots.size)
  ) {
    this.toast &&
      this.toast(
        "Aucun slot sélectionné : utilise Slots → Tout / +N pour sélectionner.",
      );
    return;
  }
  ...
  this.ensureSlotCursorsDefensive
    ? this.ensureSlotCursorsDefensive()
    : this.ensureSlotCursors && this.ensureSlotCursors();
...
PDFTool.prototype._computeColsSafe = function () {
  try {
    if (typeof this.computeGrid === "function") {
      ...
      const g =
        this.computeGrid.length >= 2
          ? this.computeGrid(this.slots, this.mode)
          : this.computeGrid();
...
function bindSlotSelectionButtons(tool) {
  ...
  btnAll &&
    (btnAll.onclick = () => {
      tool.selectAllSlots
        ? tool.selectAllSlots()
        : ((tool.selectedSlots = new Set(
            Array.from({ length: tool.slots }, (_, i) => i),
          )),
          tool.render());
    });
  btnNone &&
    (btnNone.onclick = () => {
      tool.deselectAllSlots
        ? tool.deselectAllSlots()
        : (tool.selectedSlots.clear(), tool.render());
    });
  btnPlusN &&
    (btnPlusN.onclick = () => {
      tool.selectNSlotsFromActive
        ? tool.selectNSlotsFromActive()
        : (function () {
            const n = nVal();
            const start = tool.activeSlot || 0;
            for (let k = 0; k < n; k++)
              tool.selectedSlots.add((start + k) % tool.slots);
            tool.render();
          })();
    });
  btnMinusN &&
    (btnMinusN.onclick = () => {
      tool.deselectNSlotsFromActive
        ? tool.deselectNSlotsFromActive()
        : (function () {
            const n = nVal();
            const start = tool.activeSlot || 0;
            for (let k = 0; k < n; k++)
              tool.selectedSlots.delete((start + k) % tool.slots);
            tool.render();
          })();
    });
